...
export async function saveEventResults(results: EventResult[]) {
  try {
    ...
    // Then, calculate and update points for all affected events
    const eventIds = [...new Set(results.map((r) => r.event_id))];

    for (const eventId of eventIds) {
      await calculateAndUpdateEventPoints(eventId);
    }

    return { message: 'Results saved and points calculated successfully' };
  } catch (error) {
    console.error(error);
    return { message: 'Error saving event results' };
  }
}
...
// Helper function to calculate and update points for an event
async function calculateAndUpdateEventPoints(eventId: number) {
  ...
  const user_prediction_points = userPredictions.map((userPrediction) => {
    ...
    item_points_configs.forEach((points) => {
      if (points.item_id === item_id) {
        ...
        if (points.points_type === 'EXACT' && user_exact_item == result_value) {
